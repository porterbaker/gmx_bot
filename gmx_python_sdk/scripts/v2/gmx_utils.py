...
def get_balance(config, token_address):
    """
    Get the balance of a specific token for the user wallet.

    Parameters
    ----------
    config : ConfigManager
        The GMX configuration object with wallet info.
    token_address : str
    ...
    -------
    float
        Token balance in human-readable units (adjusted by token decimals).
    """
    web3_obj = create_connection(config)
    token_contract = get_token_balance_contract(config, token_address)

    balance_raw = token_contract.functions.balanceOf(config.user_wallet_address).call()

    # Fetch decimals from the token contract
    try:
        decimals = token_contract.functions.decimals().call()
    except Exception:
        decimals = 18  # fallback if token contract has no decimals function
